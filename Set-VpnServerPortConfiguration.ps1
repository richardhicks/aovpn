...
If ($Ikev2Ports -gt '30000') {

    Write-Warning 'The number of IKEv2 ports specified exceeds the maximum limit of 30,000. Choose a lower value.'
    Return

}

If ($SstpPorts -gt '30000') {

    Write-Warning 'The number of SSTP ports specified exceeds the maximum limit of 30,000. Choose a lower value.'
    Return
...
Else {
    ...
    Invoke-Command -ScriptBlock { netsh.exe ras set wanports device = "WAN Miniport (SSTP)" rasinonly = enabled ddinout = disabled ddoutonly = disabled maxports = $SstpPorts.ToString() } | Out-Null
...
Else {
    ...
    Invoke-Command -ScriptBlock { netsh.exe ras set wanports device = "WAN Miniport (IKEv2)" rasinonly = enabled ddinout = disabled ddoutonly = disabled maxports = $Ikev2Ports.ToString() } | Out-Null
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU9vdJpbS8X2wAoCSegYmfd/By
# KmqgghfxMIIE/jCCA+agAwIBAgIQDUJK4L46iP9gQCHOFADw3TANBgkqhkiG9w0B
...
# BBQS4bWOiIQ8FswDgoccVFdc8sKK3DANBgkqhkiG9w0BAQEFAASCAYB3Qiyj9cFm
# PL3He6CwnlkyDpXOjPKf0+8deCbz4cJpidfr4flzEX0302mt1OX3rKVGXm1FaxDD
# 0Hr7yBTkAgfv+8WXJGv6WGa/PgnaZBQ6RnqQ+M/rKBtPvuIQbSFsvR1BISG5N7oH
# uMbDUhizVYBsC4a9MhoeyISgvhBOtJJKH9+XvMVMUSPbGJ4n2Oh7XA2hbX/e0TJA
# 6VlUIcX3/jl9Sg17Oh6N1CUlAgSBiF9wEbPED52FkQkZm/8ifS/zdfuUJGCbsUiZ
# g/u7p9mfiJdPmhyJtgGimfaHkcIElNaaZTsJp5UpX4Govm0nwj+sYd8hmm6hsPGh
# 4+pg1waGAVzi1MpB42NX926dtXCmXn8FlLQAf/DCnruo0N3yqG5olxDfLknlxkgZ
# WFQlK32dySxauXcND7J3Bul3Rian7G1l96/f0pOQPNEzJzhZu2HS7n2jjUXEKXTM
# qJay+tml7z/sB+iQLUK6RaqK8EvRoKtqDCDJ0m4xqmEBucCfsAECUu+hggIwMIIC
...
# BgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIyMDIxOTE3MDI0OFowLwYJKoZI
# hvcNAQkEMSIEIHX35XHSH4TCOimdukNWMzLWcIT9DBfY9mX9Q3uYZgPDMA0GCSqG
# SIb3DQEBAQUABIIBAA0+pu3z/XDJRO26fG6QDfKdqadAdM4Syrt48zgsXO7MrXIr
# nkdQ0zQXlxL3hg30dKGtTYEy2ot5Y360aWvdc+1klJZR+ane7mnoHqSKAep6N5Ky
# lM4JaUcRMGZLk2lxhrGAIUDzbknYhZU4kp6P3ODOpRFjdQopdGjmhQzGEgyexjIY
# w/aSpwpYWupdxZJXGK8ZJuxZKS8U/GMrj8aCidadmtzV/5EYjW4d71V8GkDQGzW6
# AsmPdmXneRpmMIi75FH9Xbo4Ybo+yjrw/bhMZfWKrKnXEuEGE0R3LNoPPGkowDcy
# LgjGsX0d7iQkneXnxxGhd6HvbcBAUGZcL3FTQbc=
