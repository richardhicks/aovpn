...
.SYNOPSIS
    Display IP routes configured for a VPN connection profile.

.PARAMETER ConnectionName
    Name of the VPN connection to view routing information for.

.PARAMETER DeviceTunnel
    Specifies the VPN connection is a device tunnel.

.EXAMPLE
    .\Show-VpnConnectionRoutes.ps1 -ConnectionName 'Always On VPN'

    Displays any IP routes defined on the VPN connection, if configured.

.EXAMPLE
    .\Show-VpnConnectionRoutes.ps1 -ConnectionName 'Always On VPN Device Tunnel' -DeviceTunnel

    Displays any IP routes defined on the device tunnel VPN connection, if configured.

.DESCRIPTION
    Windows does not provide the ability to view IP route configuration in the UI.
    In addition, Windows does not provide a native PowerShell command to view these settings.
    This script gives administrators the ability to easily view configured IP routing settings.
...
$Routes = ($Vpn | Select-Object -ExpandProperty Routes)

If ($Routes -eq $null) {

    Write-Warning 'No routes configured.'
    Exit
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU96Ot/bkl/7HWXuZTBj9A38/w
# SnugggquMIIFMDCCBBigAwIBAgIQBAkYG1/Vu2Z1U0O1b5VQCDANBgkqhkiG9w0B
...
# AQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFFJbRnaEOXhbE+Y0djzPydlniHZ6MA0G
# CSqGSIb3DQEBAQUABIIBAEPS0Y7ci3u2yEMwH5+zL4t4LDPiffrjHsSykZY0mCEu
# 1kuVPcrYZ2R9I+kLyUtdthf01U0uSlbz8FfitTckqGKtMA8RvMfoTdkkYUBrY4QJ
# JjYMB6J1/mH+upLGPvSj3Kmxfkertp04piCwXAOwanKszVIgDWL+WfqkKHyUDEYc
# SiGvlmUyAgAr9v1TaJmXOTRJmEWuCXq1pXQCU1XigdfrRr3a8l1JjYmWSDCkDCdP
# gEEebHhKHphHHln1Ogp+y0fGyER86qF2eDLYVli+fZvVf4Qdj+AZf4ErX3uT0TXh
# pXNyoPKuMXwmDRTlckd73Hyo6sbPiDgPwJj3awkoUxU=
