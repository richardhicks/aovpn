...
If ((Get-Service 'MSSQL$MICROSOFT##WID').Status -ne 'Running') {
    ...
    Write-Warning 'The SQL database service is not running. Exiting script.'
    Exit
...
If ($BackupDatabase) {
    ...
    If (Test-Path $BackupFile) {
        ...
        If ($Overwrite) {
            ...
            Write-Verbose "Deleting previous backup file $BackupFile..."
    ...
    $Connection.ConnectionString = 'Server=np:\\.\pipe\Microsoft##WID\tsql\query;Database=RaAcctDb;Trusted_Connection=True;'
    $Command = $Connection.CreateCommand()
    $Command.CommandText = "BACKUP database RaAcctDb TO DISK='$BackupFile' WITH DESCRIPTION = 'Full backup of the RemoteAccess accounting database taken on $((Get-Date).ToLongDateString()) at $((Get-Date).ToLongTimeString()).'"
    $Connection.Open()
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQULd7RSAL2mRSFR2t0OwtxSHsR
# v32gghfxMIIE/jCCA+agAwIBAgIQDUJK4L46iP9gQCHOFADw3TANBgkqhkiG9w0B
...
# BBRHHXiokoYw87z1DkJKFyNgdu9M3DANBgkqhkiG9w0BAQEFAASCAYC7Su0OmSgd
# g2BAOhZwZ8E3U2YsKznOkQNNgsnlDdoqbifXZK6ncPE8Pc/tZvdL05dwsKEKFMvl
# dXORkhvTciSQjmFKpTbaG1tW3zBwjLyvenDT2MGqU7uuV6rqN0h+f03FLuSgSE61
# llrrweBHOaxGMqsWk7COtjj+8mQSLcrx3EYVfCRUWamgoMlPkouxpcqV69zUyfK1
# mNhE3lehIyN463akIEC2YphAc35gr368WGJ9Wp4TZD4H+vj9/I3TmGCsiWQlpTzQ
# QH/fAHRzo8KyMIOqdZr6JydBoSAibm3+eRMxb2pErOoi1YrRhCB12hkhGnYDre7m
# 3UJK1OmamkE0DzRRKqOMSARW2G7jlqM6MONAI1CY9Pl9YPKmzMJsdZ1veHgit2u2
# jWowBvi8V/E/25Fecw90I6UKOFfqbBbhdv/fFOI/+8B8+vfsaKM7SQp3O0kbosMl
# z1qtC0ufJtdKnLpB0OdqkTOFbotKVn2HAWQCjHQuqh1nmpYlHOmUOCGhggIwMIIC
...
# BgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIyMDEwMzIwNTkyN1owLwYJKoZI
# hvcNAQkEMSIEIDzzpwJvgN7pRu9RJKtpdD/w1h9uh1h+Fw2ZnMaWKX/EMA0GCSqG
# SIb3DQEBAQUABIIBABTRrwAmUzrv1P+2gwYBhHcqmJ/MTJhscKNME5YIVXkiUgEj
# 26dktMgKtrLrEAAn7J2jfYfkJdvg5uoVCyAxeXh3FH5Kc6FT8585ylehks+Dgi/g
# RcN50+/HfRX43o4LDyNhTUwLwnDHbVuu3jXjyfEuMjJxS6/cpDZLawUUils6nEZk
# 1YkA6zUeC9C21Swc6VsrBGtDysKV40SAn75qAJUOU9uXLhJLaTC9bY8MgNolPY5c
# cSrLnb2n4KatUUGsTjneEwM10JX4o0KjltpyVrc+e6qXHcZmCrTX6v7ffw8iWqEy
# Qi2wvJ1p3grJwes+6cv1/EiaH3cohFOcy05rsHY=
