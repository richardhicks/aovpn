...
.PARAMETER IncludeNPS
    Use this parameter to back up the NPS configuration if NPS is installed on the VPN server.
...
.EXAMPLE
    .\Import-VPNServerConfiguration.ps1 -Path C:\Backup\rras.txt -IncludeNPS -NpsPath C:\Backup\nps.xml -Restart

    Imports an RRAS configuration backup file from the path C:\Backup\rras.txt and an NPS configuration backup file from the path C:\Backup\nps.txt and restarts the RemoteAccess service when complete.

.DESCRIPTION
    This script will import a previously exported Windows Server RRAS configuration backup file, and optionally a Windows Server NPS configuration backup file.
...
If ($IncludeNPS) {
    ...
    Write-Verbose "Checking for NPS backup file $NpsPath"

    If (!(Test-Path $NpsPath)) {

        Write-Warning "NPS backup file ""$NpsPath"" does not exist."
    
    }

    Else {

        $FullNPSPath = Get-ChildItem -Path $NpsPath
        Write-Verbose 'Importing NPS configuration...'
        Import-NpsConfiguration -Path $FullNPSPath.FullName
...
Write-Warning 'RADIUS shared secrets and SSTP certificate binding configuration are NOT imported by default! Update manually, if required.'

# SIG # Begin signature block
# MIIdWQYJKoZIhvcNAQcCoIIdSjCCHUYCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUjmplq04L1CexTmBxIEHb0tZ2
# DqGgghfxMIIE/jCCA+agAwIBAgIQDUJK4L46iP9gQCHOFADw3TANBgkqhkiG9w0B
...
# BBQneBxYcZmkoZ+UzMnCZiTXGulMTjANBgkqhkiG9w0BAQEFAASCAYB2DgjcMW01
# 1ILudBMmfIFm2llh1eedj61eDITIWQ4EVfhufosTNIFIGJbV/UaRPhqm1s4ZBdNF
# +AunB1W2HKGPqXol1Y4yQKnVsZWVQXNs6xQ73lQyIkrM0eeFoeqjAgyP9YifVfB6
# CytuT7k0HNaVLviTpnsfibZVHpVML3ARaiL8dBcuQzOkIFRD+brIA/2/ImZpL2yy
# 9Xa/zYy78cSqJ/UyWefRnMC1nUu0uh8Jj2sj6czo5FK53R2W+7VDmkzJaMjpcHFi
# 4rGW2Jn4UOjXBm0yFlFYb+5gevEZK9jA2RneYaostd3Ypndd6sgPIQWvYY4yFhGS
# Q43q53YffAoO7IMQwU3WYKu/keJJLC8rMYhGyNZCJYcUw7HRZ3IwBy+FaTHLyAaR
# 8dvkxOXNeCsJ7zkOnh+5t1XQ9u6TzyBvV55oXonTABGrKKqzqVAIWAyYudZURkAx
# CvAno2FSGG2pAxr6HXbVp1m/7yx/Q2m8p11Z54A7f2xYQYPgQjEvUoGhggIwMIIC
...
# BgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIyMDEyNzE5MjEwN1owLwYJKoZI
# hvcNAQkEMSIEIBQ5eXOpbt7s0/UbPeiYebtPTAWIpZJzySp0xGAQEB2zMA0GCSqG
# SIb3DQEBAQUABIIBAFDxKTNE+8GIvA+jxy+fWyAXOde7bJRSWl/Gf/18++GsJ6g/
# bdmjATx9JWlcFbxvTLIKoT+yS3KtqL0TOf7dLxHMJ7BlunXcoQx4sWTKaCHuUxyQ
# 4EdtATITVU34QECdHN4uVkgl5njN+MFS+FKonFvP+ywAp/XHVxlBuwknU046ivhA
# bTcODu4Yy5KPoxeTJP9bcYPZ9euxaMqJ61N322k8phSYjjn1AakHDVp+VJh5e2lX
# +ZjdCt/xHPs1RMAMdCS1ixwqNizMuuMNUkCVnjeRAjUa947Bd6hCixJ1a3MUtoRJ
# Os+6evFrK10M0FEx35SbXLVwCW7Y+S17Kbx/qvE=
