...
Param (
    ...
    [string]$Path = 'C:\Windows\DirectAccess\db\',    
...
# // Validate inbox accounting disabled - exit script if still configured
If ((Get-RemoteAccessAccounting).InboxAccountingStatus -eq 'Enabled' -and !($DisableInboxAccounting)) {

    Write-Warning 'Inbox accounting has not been disabled. Exiting script.'
    Exit

}

$Database = Join-Path $Path -ChildPath 'RaAcctDb.mdf'

# // Validate inbox accounting database location. Exit if it doesn't exist
If (!(Test-Path $Database)) {

    Write-Warning "Inbox accounting database does not exist in $Path. Existing script."
    Exit
...
If ($BackupDatabase) {
    ...
    $BackupPath = Join-Path $Path -ChildPath 'RaAcctDb.bak'
    ...
    $command.CommandText = "BACKUP database RaAcctDb TO DISK='$BackupPath' WITH DESCRIPTION = 'Full backup before removing inbox accounting database.'"
    $connection.Open()
    $command.ExecuteNonQuery() | Out-Null
    $connection.close()

    Write-Verbose "Inbox accounting database backed up to $BackupPath."
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUxIc37TwJlCiU96NbftekpTR9
# xEygggquMIIFMDCCBBigAwIBAgIQBAkYG1/Vu2Z1U0O1b5VQCDANBgkqhkiG9w0B
...
# AQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFG9qQizXUSTjTe5GasrtpkxYd8QIMA0G
# CSqGSIb3DQEBAQUABIIBAAQTQQSWaqO4lSt7k2xpkdEempRkX7yj0MJBbwxKLmUg
# cJDVgN/aonOI3rDmlrOPN4D6ZmIGYFR+VeCQG5JqQM4k+Wk1uRIFGxcaNgBzwewO
# TqBxoD2gulIK/TvWAOF+5kSOA31Fro0x935fQ5XDC8zU455cOBlA3j3Ye1qNGHKl
# udvLRAtWTwCTY1h7zjY4wFIYCa3xgfF6y8lBg34UseCmPTziJk000GTlRrnlFrmZ
# HoDTG8JcQeu1/6qtCjlx4sxHNPNEDc14NMs/5qhNS7NCOXLEX+DDOixzPVcIQXb2
# CE/UYOfQa/XcWfNwjHDJUB6Hb6YYEqdHcOWe/e5w/1o=
