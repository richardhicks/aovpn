...
.PARAMETER RrasPath
    File name and path for the RRAS configuration export file. The file is placed on the current users' desktop by default.

.PARAMETER NpsPath
    File name and path for the NPS configuration export file. The file is placed on the current users' desktop by default.

.PARAMETER IncludeNps
    Export the NPS configuration if NPS is configured on the VPN server.

.EXAMPLE
    .\Export-VPNServerConfiguration.ps1 -RrasPath C:\Backup\rras.txt

.EXAMPLE
    .\Export-VPNServerConfiguration.ps1 -Path C:\Backup\rras.txt -IncludeNps -NpsPath C:\Backup\nps.xml

.DESCRIPTION
    This script will export the Windows Server RRAS configuration to text file. Optionally, using the -IncludeNps parameter, the script will export the NPS configuration if configured on the local VPN server. The output file(s) can be imported to restore the configuration.
...
Param(
    ...
    [string]$NpsPath = "$($env:USERPROFILE)\Desktop\$env:computername" + "_nps.xml",
    [switch]$IncludeNps
...
If ($IncludeNps) {
    ...
    Export-NpsConfiguration -Path $NpsPath.ToLower()
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUl8FvPPumbmt1nmcRkuJT2Q7S
# Bf6gghfxMIIE/jCCA+agAwIBAgIQDUJK4L46iP9gQCHOFADw3TANBgkqhkiG9w0B
...
# BBQPl6EQE6ourB3Rz1fgWo5sLE5byTANBgkqhkiG9w0BAQEFAASCAYA4/ZNtPvgT
# FPMT9/RHy65JsKIw9Q2Ei6bAz5ChczAI3vpdlSv+fKKdACnLwVTF5ywIvpXgXHKF
# +ufH5eNTwPiqBcvphmFc6Xk5R8MvLkhE56zsxY4iMYuUH4mADCm8OI+ZJYlxAc4G
# qxqVYJ/2ael+QpQXOSdMDIx6VgWBbL5jtO/uOcbrKD/WXRnKzXFPzYNrUC3B01lU
# zARCfHy7Va2J55co8HiIOOC8sGyGtIlq13Hd8iXqipBxdA5QjlVmF8yflGO4o+V2
# 5myH52BRVfoWshjyb05pzKAxHite3Q2aTIBdqOBPKsZ6koRS7xMHMAtGrhyfsg8C
# pYOVUQZ7DpPfs9qV6MVatZ4xOnyC/ycsVAl9iG2OapXGf+wFdee9gQW4GYo3kCAp
# nyz/2LLl0i2MtkK4Ncr+T9iBjO9stK0hOADHZnvEAYoGLfw8wAqU04aoBAjaEey5
# mtmeCxTrCVsU0lmZj6VYI66wvKREaVR1St12t/qI50xWdw4elacKIUqhggIwMIIC
...
# BgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIyMDEyNzE5MjExMlowLwYJKoZI
# hvcNAQkEMSIEIFj6XeEmXewXnsGhi9l5lsby6Y2Fs5GhiOiy+kGq5cXeMA0GCSqG
# SIb3DQEBAQUABIIBAGJQJGm1/eg+enhVGWy/iZD84SO9kbtd1Oyc7xTGSvu/VxP9
# 6gHlpb6cR/SCJhxG6vX5GNqXtIOH+TLvY+Yzl+LfVPiJAoj8L56uzETD4h2NawqD
# PqfSqX385myqCEgMexDbhsIToz/E96fY4LRtBctlf5aaAZaGpvZ11s+dSoAixP21
# rwvGs48+PwiJ8XW0Rum0uG2fLgaLHzpiQSFoaJ5KSJ81iFh0oBUXvPfM8m97/heE
# b+7XQUAtyTidea40KzEM46lebeH+W2d8x3RTy6Jne/I1zaHFGThlmJnLYKT2R9TX
# 0G/ns+hM53YURivOeWldwxEF3cWZMkkciCFXjEc=
